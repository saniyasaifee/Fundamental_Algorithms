/* Height Balanced tree with back pointers instead of stack*/
...
typedef struct tr_n_t { key_t key;
    struct tr_n_t *left;
    struct tr_n_t *right;
    struct tr_n_t *parent; /*This field points to upper neighbor of the node. Thus if a node is the root of the tree then parent point to NULL*/
    int height;
}tree_node_t;
...
void left_rotation(tree_node_t *n){
    ...
    if(NULL != n->left->right)
        n->left->right->parent = n->left;
    n->left->left = tmp_node;
    if(NULL != n->left->left)
        n->left->left->parent = n->left;
    n->left->key = tmp_key;
    n->left->parent = n->right->parent = n;/* set the backpointer*/
 }
...
void right_rotation(tree_node_t *n){
    ...
    if(NULL != n->right->left)
        n->right->left->parent = n->right;
    n->right->right = tmp_node;
    if(NULL != n->left->left)
        n->right->right->parent = n->right;
    n->right->key = tmp_key;
    n->right->parent = n->left->parent = n;/* set the backpointer*/
}
...
int insert(tree_node_t *tree, key_t new_key, object_t *new_object){
    ...
    else
    {
        ...
        {   tree_node_t *old_leaf, *new_leaf;
            ...
            new_leaf->right  = NULL;
            new_leaf->height = 0;
            old_leaf->parent = new_leaf->parent = tmp_node;/* set the backpointer*/
            if( tmp_node->key < new_key ){
        ...
        /* rebalance */
        finished = 0;
        while( NULL != tmp_node->parent && !finished){
...
object_t* delete1(tree_node_t *tree, key_t delete_key){
    ...
    while ((nextop = getchar())!= 'q') {
        if(nextop == 'i'){
            ...
            *insobj = 10*inskey+2;
            ...
            if(success == 0){
                printf("insert successful, key = %d, object_value = %d, height is %d\n", inskey, *insobj,searchtree->height);
            }
            else{
            ...
            if(nextop =='d'){
                int delkey, *delobj;
                scanf("%d", &delkey);
                delobj = delete1(searchtree, delkey);
                if(delobj == NULL)
